...
SegM8::SegM8(uint8_t pinCS, uint8_t deviceCount)
    : _spi(pinCS, deviceCount), _deviceCount(deviceCount) {}

SegM8::SegM8(uint8_t pinCS, uint8_t pinDI, uint8_t pinCLK, uint8_t deviceCount)
    : _spi(pinCS, pinDI, pinCLK, deviceCount), _deviceCount(deviceCount) {}
...
void SegM8::display(unsigned long number, uint8_t position, uint8_t width,
  ...
  if (number == 0) {
    ...
    for (uint8_t i = index; i > 0; i--) {
      ...
      number /= radix;
      if ((number == 0 && !(flags & SEGM8_PAD_ZEROS)) ||
          i + width < sizeof(_buffer) - 1) {
        index = i;
        break;
      }
    }
...
void SegM8::display(float number, uint8_t position, uint8_t width,
  ...
  if (rightPart == 0) {
    ...
    for (uint8_t i = index; i > 0; i--) {
      ...
      rightPart /= 10;
      if ((rightPart == 0 && !(flags & SEGM8_PAD_ZEROS)) ||
          i + width < sizeof(_buffer) - 1) {
        index = i;
        break;
      }
  ...
  if (leftPart == 0) {
    ...
    for (uint8_t i = index; i > 0; i--) {
      ...
      leftPart /= 10;
      if ((leftPart == 0 && !(flags & SEGM8_PAD_ZEROS)) ||
          i + width < sizeof(_buffer) - 1) {
        index = i;
        break;
      }
...
void SegM8::display(String string, uint8_t position, uint8_t width,
  ...
  if (flags && SEGM8_ALIGN_LEFT) {
    beginPosition = min(position, _spi.chainLength());
    endPosition =
        min(_spi.chainLength(), (position + min(width, string.length())));
  } else { // SEGM8_ALIGN_RIGHT
    beginPosition =
        min((position + (width - string.length())), _spi.chainLength());
    endPosition = min(_spi.chainLength(), (position + width));
  }
